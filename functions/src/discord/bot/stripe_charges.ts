...
const iterateCharges = (apiRes: Stripe.ApiList<Stripe.Charge>, startDate: Dayjs, endDate: Dayjs): Charge[] => {
  ...
  apiRes.data.forEach((charge: Stripe.Charge) => {
    ...
    if (dayjs(curDate).isAfter(startDate) && dayjs(curDate).isBefore(endDate)) {
      allCharges.push({stripeId: charge.id, date: curDate.toISOString(), status, ammount: Math.abs(ammount)});
    }
  });
  return allCharges;
};
...
const getAllUsersAndProjects = async () => {
  ...
  for (let index = 0; index < res.users.length; index++) {
    ...
    if (allUsers[user.userId]) {
      ...
      if (projects) {
        allUsers[user.userId].projectsData = projects.concat(await getAllProjects(user.userId));
      } else {
        allUsers[user.userId].projectsData = await getAllProjects(user.userId);
      }
    } else {
      allUsers[user.userId] = {...user, projectsData: await getAllProjects(user.userId)};
    }
  }
  return allUsers;
};
...
const updateCurrentIcome = (data: {userId: string, projectId: string, charges: Charge[], incomes: Income[]} | undefined) => {
  if (!data) return null;
