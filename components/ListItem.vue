    ...
    <div class="flex w-full">
      <div class="w-2/5 md:w-1/4">
        <div class="relative square">
          ...
          <div v-if="votes" class="absolute bottom-0 right-0 px-3 text-center border-4 border-white cursor-pointer tumb_up bg-royalblue-700" @click="$emit('voted')">
            👍<br>
            {{ votes }}
          </div>
        </div>
      ...
      <div class="w-3/5 p-2 overflow-hidden md:w-3/4">
        ...
        <p v-if="name" class="z-10 text-sm cursor-pointer text-orchid-300 md:mb-1" @click="$emit('name')">
          @{{ name }}
        </p>
